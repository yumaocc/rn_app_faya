...
export async function loadCommissionTop(
  beginTime: DateTimeString,
  endTime: DateTimeString,
): Promise<CommissionTop> {
  return await get<CommissionTop>('/sku/commission/top/ten', {
    params: {beginTime, endTime},
  });
}

export async function loadSalesTop(
  beginTime: DateTimeString,
  endTime: DateTimeString,
): Promise<SaleTop> {
...
export async function getCommissionOrder(
  param: SearchParam,
): Promise<PagedData<CommissionDetail[]>> {
...
// 预计收入列表
export async function getExpectCommissionOrder(
  param: SearchParam,
): Promise<PagedData<CommissionDetail[]>> {
  return await getPaged<CommissionDetail[]>(
    '/sku/commission/details/paid/page',
    {params: param},
  );
}
