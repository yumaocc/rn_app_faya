...
import {numberToString, stringToNumber} from '../../../helper';
...
const InputNumber: React.FC<InputNumberProps> = props => {
  ...
  function renderAddControl() {
    ...
    if (canAdd) {
      return (
        <TouchableOpacity activeOpacity={0.8} onPress={() => handleStep(true)}>
          <View style={styles.action}>
            <Icon name="pushpin" size={20} color={globalStyleVariables.TEXT_COLOR_PRIMARY} />
          </View>
        </TouchableOpacity>
      );
    } else {
      return (
        <View style={styles.action}>
          <Icon name="pushpin" size={20} color={globalStyleVariables.TEXT_COLOR_TERTIARY} />
        </View>
      );
    }
  }
    ...
    if (canSubtract) {
      return (
        <TouchableOpacity activeOpacity={0.8} onPress={() => handleStep(false)}>
          <View style={styles.action}>
            <Icon name="delete" size={20} color={globalStyleVariables.TEXT_COLOR_PRIMARY} />
          </View>
        </TouchableOpacity>
      );
    } else {
      return (
        <View style={styles.action}>
          <Icon name="delete" size={20} color={globalStyleVariables.TEXT_COLOR_TERTIARY} />
        </View>
      );
    }
  }
